...
import { checkFieldsValidity } from '../utils.js';

const container = document.querySelector('body div.container');

const editViewTemplate = (item) => html`
    ...
    <form id=${item._id} @submit=${onSubmit}>
        <div class="row space-top">
            <div class="col-md-4">
...
async function onSubmit(e) {
    ...
    const { make, model, year, description, price, img, material } = Object.fromEntries(form);
    const id = e.currentTarget.id;

    const isFormValid = checkFieldsValidity({ make, model, year, description, price, img, material });
